...
<div class="mx-auto max-w-3xl px-4 py-12">
	<a href={localizeHref('/')} class="mb-8 inline-block text-sm opacity-60 hover:opacity-100">
		&larr; {m.common_back()}
	</a>

	<div class="mb-8 overflow-hidden bg-muted">
		<img src={data.artwork.imageUrl} alt={localize(data.artwork.title, locale)} class="w-full object-contain" />
	</div>
	...
	{#if data.artwork.description}
		<p class="mt-6 leading-relaxed">{localize(data.artwork.description, locale)}</p>
	{/if}
</div>
