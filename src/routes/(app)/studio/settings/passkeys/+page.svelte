...
<div class="max-w-2xl space-y-6">
	...
	{#if feedback}
		<p class="text-sm {feedback.type === 'error' ? 'text-destructive' : 'text-green-500'}">
			{feedback.message}
		</p>
	{/if}
					...
					{#each passkeys as pk}
						<li class="flex items-center justify-between rounded-md border px-3 py-2">
							...
							<div class="flex items-center gap-2">
								{#if pk.backedUp}
									<Badge variant="outline" class="text-xs">{m.studio_passkey_backed_up()}</Badge>
								{/if}
								<Button
									variant="ghost"
									size="icon"
...
<Dialog.Root open={!!deleteTarget} onOpenChange={(o) => { if (!o) deleteTarget = null; }}>
	<Dialog.Content>
		...
		<Dialog.Footer>
			...
			<Button variant="destructive" onclick={deletePasskey}>{m.common_delete()}</Button>
		</Dialog.Footer>
	</Dialog.Content>
</Dialog.Root>
