<script lang="ts">
	...
	function formatDate(date: Date) {
		return new Intl.DateTimeFormat('de-DE', { dateStyle: 'short' }).format(new Date(date))
	}
</script>
						...
						<div class="flex items-center gap-1">
							...
							{#if data.canDelete}
								<Button
									...
									onclick={() => { deleteTarget = p.id; deleteOpen = true }}
								>
									<Trash2 class="size-4" />
								</Button>
							{/if}
...
<Dialog.Root bind:open={deleteOpen}>
	<Dialog.Content>
		...
		<Dialog.Footer>
			<Button variant="outline" onclick={() => { deleteOpen = false; deleteTarget = null }}>
				{m.common_cancel()}
			</Button>
			<form method="POST" action="?/delete" use:enhance={() => () => { deleteOpen = false; deleteTarget = null }}>
