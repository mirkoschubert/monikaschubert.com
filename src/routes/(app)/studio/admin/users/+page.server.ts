import { getAllUsers, deleteUser, setUserRole, createUser, updateUser, banUser, setUserPassword } from '$lib/server/services/user'
...
export const load: PageServerLoad = async ({ locals }) => {
	...
	const [users, canCreate, canUpdate, canDelete, canSetRole, canBan, canSetPassword] = await Promise.all([
		getAllUsers(),
		hasPermission(role, 'user', 'create'),
		hasPermission(role, 'user', 'update'),
		hasPermission(role, 'user', 'delete'),
		hasPermission(role, 'user', 'set-role'),
		hasPermission(role, 'user', 'ban'),
		hasPermission(role, 'user', 'set-password'),
	])
	return { users, canCreate, canUpdate, canDelete, canSetRole, canBan, canSetPassword }
}
...
export const actions: Actions = {
	...
	setRole: async ({ request, locals }) => {
		...
		if (role !== 'admin' && role !== 'editor') return fail(400, { error: 'Invalid role' })
	...
	ban: async ({ request, locals }) => {
		...
		if (!id) return fail(400, { error: 'Missing user id' })
		if (id === locals.user?.id) return fail(400, { error: 'Cannot ban yourself' })
	...
	create: async ({ request, locals }) => {
		...
		const password = data.get('password')?.toString()
		const role = data.get('role')?.toString()
		if (!name || !email || !password) return fail(400, { error: 'Missing fields' })
		if (role !== 'admin' && role !== 'editor') return fail(400, { error: 'Invalid role' })
