<script lang="ts">
  ...
  let editTarget = $state<{ id: string; name: string; email: string } | null>(
    null
  )
  let banTarget = $state<{ id: string; name: string; banned: boolean } | null>(
    null
  )
  ...
  const hasActions = (userId: string) =>
    userId !== data.user?.id &&
    (data.canUpdate || data.canDelete || data.canBan || data.canSetPassword)
</script>
              ...
              <form method="POST" action="?/setRole" use:enhance>
                ...
                >
                  ...
                  <option value="editor" selected={u.role === 'editor'}
                    >{m.admin_users_role_editor()}</option
                  >
                </select>
              </form>
            {:else}
              ...
              <DropdownMenu.Root>
                ...
                <DropdownMenu.Content align="end">
                  ...
                  {#if data.canSetPassword}
                    <DropdownMenu.Item
                      onSelect={() =>
                        (passwordTarget = { id: u.id, name: u.name })}
                    >
    ...
    >
      ...
      <Dialog.Footer>
        <Button
          variant="outline"
          type="button"
          onclick={() => (createOpen = false)}>{m.common_cancel()}</Button
        >
      ...
      >
        ...
        <Dialog.Footer>
          <Button
            variant="outline"
            type="button"
            onclick={() => (editTarget = null)}>{m.common_cancel()}</Button
          >
      ...
      >
          ...
          <div class="flex gap-2">
            <div class="relative flex-1">
              ...
              <button
                ...
                class="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground"
              >
                {#if showNewPassword}<EyeOff class="size-4" />{:else}<Eye
                    class="size-4"
                  />{/if}
              </button>
            </div>
        ...
        <Dialog.Footer>
          <Button
            variant="outline"
            type="button"
            onclick={() => (passwordTarget = null)}>{m.common_cancel()}</Button
          >
          <Button type="submit">{m.common_save()}</Button>
        </Dialog.Footer>
      </form>
  ...
  <Dialog.Content>
    <Dialog.Header>
      <Dialog.Title
        >{banTarget?.banned
          ? m.admin_users_unban()
          : m.admin_users_ban()}</Dialog.Title
      >
    ...
    <Dialog.Footer>
      <Button variant="outline" onclick={() => (banTarget = null)}
      ...
      {#if banTarget}
        ...
        >
          ...
          <input
            type="hidden"
            name="banned"
            value={(!banTarget.banned).toString()}
          />
          <Button
            type="submit"
  ...
  <Dialog.Content>
    <Dialog.Header>
      ...
      <Dialog.Description>
        ...
        {#if deleteTarget}<span class="font-medium">
