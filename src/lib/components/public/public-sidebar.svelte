<script lang="ts">
	...
	import { Images, BookOpen, Mail, CalendarDays, ScrollText, ShieldCheck } from '@lucide/svelte';
...
<Sidebar.Root collapsible="icon">
				...
				<Sidebar.MenuButton size="lg" tooltipContent="Monika Schubert">
					{#snippet child({ props })}
						<a href={localizeHref('/')} {...props}>
							<div class="bg-primary text-primary-foreground flex size-8 shrink-0 items-center justify-center rounded-md text-xs font-bold">
								MS
							</div>
							<div class="flex flex-col leading-tight">
								<span class="font-semibold">Monika Schubert</span>
								<span class="text-xs text-muted-foreground">{m.artist_tagline()}</span>
							</div>
						</a>
					{/snippet}
				</Sidebar.MenuButton>
					...
					{#if isGalleryPage}
						{#each CATEGORIES as cat, i}
							<Sidebar.MenuItem>
								...
								>
									{#snippet child({ props })}
										<a href={localizeHref(`/?category=${cat.value}`)} {...props}>
											<span class="font-mono text-xs tabular-nums opacity-50 w-4 shrink-0">{String(i + 1).padStart(2, '0')}</span>
											<span>{cat.label()}</span>
										</a>
									{/snippet}
								</Sidebar.MenuButton>
	...
	<Sidebar.Footer>
		...
		<div class="px-2 py-1 group-data-[collapsible=icon]:hidden">
			<p class="text-xs text-muted-foreground">&copy; {new Date().getFullYear()} Monika Schubert</p>
		</div>
	</Sidebar.Footer>
