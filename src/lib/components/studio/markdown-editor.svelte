<script lang="ts">
  ...
  onMount(async () => {
    ...
    const { defaultKeymap, historyKeymap, history, indentWithTab } =
...
<textarea {name} class="sr-only" aria-hidden="true" readonly tabindex="-1"
  >{value}</textarea
>
...
<div class="space-y-1.5">
  <!-- Toolbar -->
  <div
    class="flex flex-wrap items-center gap-0.5 rounded-md border border-border bg-muted/30 px-1.5 py-1"
  >
    <button
      type="button"
      title="Bold"
