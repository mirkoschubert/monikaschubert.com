<script lang="ts">
  ...
  const breadcrumbLabel = $derived(() => {
    ...
    if (path.startsWith('/studio/admin/sessions'))
      return m.admin_sessions_title()
...
<header class="flex h-14 shrink-0 items-center gap-2 border-b px-4">
  ...
  <nav class="flex items-center gap-1 text-sm text-muted-foreground">
    <a href="/studio/dashboard" class="transition-colors hover:text-foreground"
      >Studio</a
    >
    <span>/</span>
  ...
  <div class="ml-auto flex items-center gap-3">
    ...
    <form
      id="signout-form"
      method="POST"
      action="/studio/dashboard?/signOut"
      class="hidden"
    ></form>
    <button
      onclick={() =>
